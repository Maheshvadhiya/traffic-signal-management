...
const buildInitialState = (): Record<Direction, DirectionState> => ({
  ...
  'South → North': { label: 'South → North', signal: 'red', countdown: GREEN_TIME * 3 },
  'East → West': { label: 'East → West', signal: 'red', countdown: GREEN_TIME * 2 },
  'West → East': { label: 'West → East', signal: 'red', countdown: GREEN_TIME },
});
...
const TrafficSignalScreen: React.FC = () => {
  ...
  const startTraffic = () => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      const activeIndex = Math.floor(tickRef.current / GREEN_TIME) % 4;
